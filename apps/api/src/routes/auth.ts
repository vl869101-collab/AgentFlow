...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.post("/register", { config: { rateLimit: { max: isDevOrTest ? 10000 : 10, timeWindow: "1 hour" } } }, async (request, reply) => {
  ...
  app.post("/login", { config: { rateLimit: { max: isDevOrTest ? 10000 : 20, timeWindow: "15 minutes" } } }, async (request, reply) => {
